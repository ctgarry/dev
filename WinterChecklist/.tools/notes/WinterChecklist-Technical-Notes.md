# WinterChecklist — A Practical Guide for New WoW Lua Developers (v2)

_Last updated: 2025-10-13 08:18_

This is a **living guide** to building and maintaining the WinterChecklist addon. It consolidates **everything taught in the retired thread** into one coherent, teachable document. It’s written by a veteran WoW addon developer for a newcomer, with Classic Era first and simple shims for Retail.
...
- Per-user path: `%LOCALAPPDATA%\hererocks\wcl54`  
  ...
  ```
- Alternative fixed path: `D:\tools\hererocks\wcl54` (adjust the wrapper accordingly).
...
3. **Run the addon in game** → `/reload` → `/wcl`
...
- **`WinterChecklist.lua`** — addon entry points, state, top-level orchestration (keep general; *no slash handlers here*).
- **`lua/slash.lua`** — all slash commands (e.g., `/wcl`) live here and call API on the namespace.
