...
local function WCL_Counts()
  if not NS.Tasks or not NS.Tasks.GetAll then
    return 0, 0
  end
...
local hasLDB, LDB = pcall(function()
  return LibStub("LibDataBroker-1.1")
end)
local hasIcon, Icon = pcall(function()
  ...
  local broker = LDB:NewDataObject(ADDON, {
    ...
    OnClick = function(_, button)
      ...
      elseif IsShiftKeyDown() then
        ...
        if NS.Print then
          NS:Print(WinterChecklistDB.debug and (L.DEBUG_ENABLED or "Debug ON") or (L.DEBUG_DISABLED or "Debug OFF"))
        end
      else
        if NS.ToggleUI then
          NS.ToggleUI()
