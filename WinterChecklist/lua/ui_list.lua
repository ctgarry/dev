...
function M:Init(parent)
  ...
  UIDropDownMenu_Initialize(freq, function()
    local function add(txt, val)
      ...
      info.func = function()
        M.freqValue = val
        UIDropDownMenu_SetText(freq, txt)
        M:Refresh()
      end
      info.checked = (M.freqValue == val)
  ...
  btnClear:SetPoint("RIGHT", btnAdd, "LEFT", -6, 0)
  btnClear:SetText(L.CLEAR or "Clear")
  ...
  btnImport:SetPoint("RIGHT", btnClear, "LEFT", -6, 0)
  btnImport:SetText(L.IMPORT or "Import")
  btnImport:SetScript("OnClick", function()
    U.ShowTextPopup(L.IMPORT_TITLE or "Paste Import", "", function(s)
      if s and s ~= "" then
  ...
  btnExport:SetPoint("RIGHT", btnImport, "LEFT", -6, 0)
  ...
  -- Widths
  if self.search and self.search.SetWidth then
    self.search:SetWidth(EDITW)
  end
  if self.btnExport then
    self.btnExport:SetWidth(BTN_W)
  end
  if self.btnImport then
    self.btnImport:SetWidth(BTN_W)
  end
  if self.btnClear then
    self.btnClear:SetWidth(BTN_W)
  end
  if self.btnAdd then
    self.btnAdd:SetWidth(BTN_W)
  end
  if self.freq and UIDropDownMenu_SetWidth then
...
function M:Refresh()
  ...
  for i, rec in ipairs(t) do
    if passFilter(rec, needle, wantInc, wantFreq) then
      ...
      r.chk:SetScript("OnClick", function(self2)
        NS.Tasks:ToggleDone(i, self2:GetChecked())
      end)
...
function M:Layout(parent)
  ...
  reset(self.search)
  reset(self.freq)
  reset(self.onlyIncomplete)
  reset(self.btnExport)
  reset(self.btnImport)
  reset(self.btnClear)
  reset(self.btnAdd)
  if self.scroll and self.scroll.ClearAllPoints then
