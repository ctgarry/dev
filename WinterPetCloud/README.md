...
- **Modular Lua stack** (`WinterPetCloud.lua` bootstrap + `lua/` modules for UI, tasks, slash commands, minimap, options)
- **Dual TOCs** (`WinterPetCloud_Mainline.toc` for Retail, `WinterPetCloud_Vanilla.toc` for Classic Era)
...
- Slash commands: `/wpc`, `/wpc toggle`, `/wpc options`, `/wpc minimap`, `/wpc debug`, `/wpc export`, `/wpc import`, `/wpc help`
...
```
WinterPetCloud.lua
WinterPetCloud_Mainline.toc     # Retail
WinterPetCloud_Vanilla.toc      # Classic Era
...
- `.tools/notes/WinterPetCloud-Technical-Notes.md` - living technical guide for building and maintaining the addon
...
- **Zip: Retail -> CurseForge** - makes `.dist/WinterPetCloud-<version>-Retail.zip`
- **Zip: Classic Era -> CurseForge** - makes `.dist/WinterPetCloud-<version>-ClassicEra.zip`
- **Install: Retail (live AddOns)** - copies files into `.\_retail_\Interface\AddOns\WinterPetCloud\`
- **Install: Classic Era (live AddOns)** - copies files into `.\_classic_era_\Interface\AddOns\WinterPetCloud\`

> Tasks call `.tools/build.ps1` which:
> - Picks the correct TOC for the flavor
> - Renames it to `WinterPetCloud.toc` in the destination
> - Copies the project files with sane include/exclude rules
> - (Zip mode) packages with a proper `WinterPetCloud/` folder at the archive root
...
- Retail reads `WinterPetCloud_Mainline.toc`
- Classic Era reads `WinterPetCloud_Vanilla.toc`

The build script copies only the chosen TOC and writes it out as `WinterPetCloud.toc` in the zip/install output.
...
/wpc debug
/wpc export
/wpc import <paste or leave empty for popup>
/wpc help
```

A small window labeled **WinterPetCloud** appears; drag to move. Zone name updates as you travel.
...
- Keep `WinterPetCloud.lua` focused on bootstrap + glue; place feature work in purpose-built modules under `lua/`.
